...
function addCrewMember(member: string, spaceship: { name: string, pilot: string, crewLimit: number, crew: string[] }) {
...
function sendToMission(spaceship: { name: string, pilot: string, crewLimit: number, crew: string[], inMission: boolean }) {
    if (spaceship.inMission) {
        ...
        alert(`${spaceship.name} não tem a quantidade de tripulantes necessária. Tripulantes: ${spaceship.crew.length}`)
    } else {
...
function firstMenuOption() {
    ...
    const crewLimit = Number(prompt(`Quantos tripulates a ${name} suporta?`))
